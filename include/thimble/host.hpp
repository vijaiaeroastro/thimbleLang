...
template<class T>
Result<Bare<T>> convert_argument(const Value& value) {
  using U = Bare<T>;
  if constexpr (std::is_same_v<U, Value>) {
    ...
    static_assert(std::is_same_v<U, void>,
                  "Thimble typed bindings support bool, int, int64_t, double, string and Value arguments");
  }
}

template<std::size_t I, class... Args>
Result<std::tuple<Bare<Args>...>> convert_arguments(const std::vector<Value>& values) {
  using Tuple = std::tuple<Bare<Args>...>;
  if constexpr (I == sizeof...(Args)) {
    return Tuple{};
  } else {
    auto converted = convert_argument<std::tuple_element_t<I, std::tuple<Args...>>>(values[I]);
...
template<class R>
Result<Value> convert_return(R&& result) {
  using U = Bare<R>;
  if constexpr (std::is_same_v<U, Value>) {
    return std::forward<R>(result);
  } else if constexpr (std::is_same_v<U, Result<Value>>) {
    return std::forward<R>(result);
  } else {
...
Result<Value> invoke_free(R(*function)(Args...), const std::vector<Value>& values) {
  if (values.size() != sizeof...(Args)) {
    return make_error(ErrorCategory::type, "arity_mismatch", "wrong argument count");
  }
...
template<class R, class C, class... Args>
Result<Value> invoke_const_member(R(C::*function)(Args...) const, const C& object,
                                  const std::vector<Value>& values) {
  if (values.size() != sizeof...(Args)) {
    return make_error(ErrorCategory::type, "arity_mismatch", "wrong argument count");
  }
...
struct ObjectDescriptor {
  struct Property {
    ...
    std::function<Result<Value>(const std::shared_ptr<void>&, const Value&)> setter;
  };
  struct Method {
    std::size_t arity=0;
    std::function<Result<Value>(const std::shared_ptr<void>&, const std::vector<Value>&)> call;
  };
  std::string name;
...
template<class T> class ObjectType {
  ...
  explicit ObjectType(std::shared_ptr<ObjectDescriptor> descriptor):descriptor_(std::move(descriptor)){}
  friend class HostContext;
public:
  const std::shared_ptr<ObjectDescriptor>& descriptor() const { return descriptor_; }
  const std::string& name() const { return descriptor_->name; }

  template<class R>
  ObjectType& property(std::string name, R(T::*getter)() const) {
    if (descriptor_->methods.count(name)) throw std::logic_error("object member name is already a method");
  ...
  ObjectType& property(std::string name, R(T::*getter)() const, void(T::*setter)(S)) {
    property(name, getter);
    auto& property=descriptor_->properties[name];
    property.setter=[setter](const std::shared_ptr<void>& instance,const Value& value){
      auto converted=detail::convert_argument<S>(value);
      if (!converted) return Result<Value>(converted.error());
      auto object=std::static_pointer_cast<T>(instance);
  ...
  ObjectType& method(std::string name, R(T::*method)(Args...)) {
    if (descriptor_->properties.count(name)) throw std::logic_error("object member name is already a property");
    descriptor_->methods[name]={sizeof...(Args),[method](const std::shared_ptr<void>& instance,const std::vector<Value>& values){
      auto object=std::static_pointer_cast<T>(instance);
...
public:
  ...
  HostContext& bind_method(std::string name, C& object, R(C::*function)(Args...)) {
    auto arity = sizeof...(Args);
    return bind_function(std::move(name), arity,
      [&object, function](const std::vector<Value>& values) {
        return detail::invoke_member(function, object, values);
      });
  }
  /// Bind a const member function on an existing object.
  template<class C, class R, class... Args>
  HostContext& bind_method(std::string name, const C& object, R(C::*function)(Args...) const) {
    auto arity = sizeof...(Args);
    return bind_function(std::move(name), arity,
      [&object, function](const std::vector<Value>& values) {
        return detail::invoke_const_member(function, object, values);
      });
  }
  /// Define a reusable descriptor for an opaque C++ object type.
  template<class T>
  ObjectType<T> define_object_type(std::string name) {
    return ObjectType<T>(std::make_shared<ObjectDescriptor>(ObjectDescriptor{std::move(name),{}, {}}));
  }
  ...
  HostContext& bind_object(std::string name, std::shared_ptr<T> object,
    ...
    values_[std::move(name)] = Value::object(std::move(object), type.descriptor(), type.name());
    return *this;
  }
